def test_exact_krylov_generates_basis_h2_sto3g():
    import numpy as np
    ...
    energies, basis_states, min_energy_history = QCANT.exact_krylov(
        symbols=symbols,
        geometry=geometry,
        n_steps=2,
        active_electrons=2,
    ...
    assert np.isclose(min_energy_history[-1], np.min(energies), atol=1e-8)
